...
int main(void)
{
    ...
    tree = splaytree_create(compare);
    if (tree == NULL)
    {
        perror("splaytree_create");
        exit(EXIT_FAILURE);
    }
    ...
    for (int iter = 0; iter < NELEMS; iter++)
    {
        ...
        item = splaytree_insert(tree, data);
        if (item == NULL)
        {
            perror("splaytree_insert");
            exit(EXIT_FAILURE);
        }
        if (data == item)
        {
    ...
    // Search records
    data->key = NELEMS / 2;
    item = splaytree_search(tree, data);
    if (item == NULL)
    {
        printf("%d not found\n", data->key);
    }
    ...
    splaytree_walk(tree, print, (int []){10});

    // Delete first 10 records
    for (data->key = 0; data->key < 10; data->key++)
    {
        item = splaytree_delete(tree, data);
        if (item != NULL)
        {
            printf("%d %s deleted\n", item->key, item->value);
            delete(item);
