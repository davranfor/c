...
void *hashmap_insert(hashmap *map, void *data)
{
    ...
    hash = map->hash(data) % map->capacity;
    tail = map->list + map->capacity;
    node = map->list + hash;
    if (tail->data != NULL) {
        if (node->data != NULL) {
    ...
    node->data = data;
    map->size++;
    if (map->size > map->capacity - map->capacity / 4) {
        tail->data = hashmap_create(map->comp, map->hash, map->capacity);
        if (tail->data == NULL) {
            return NULL;
        }
    }
...
{   
    ...
    if (map->size > 0) {
        for (iter = 0; iter < map->capacity; iter++) {
            node = map->list + iter;
            if (node->data != NULL) {
                if (func != NULL) {
                    func(node->data);
