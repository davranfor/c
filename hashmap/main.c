...
int main(void)
{
    ...
    map = hashmap_create(comp_key, hash_key, ELEMS / 100);
    if (map == NULL) {
        perror("hashmap_create");
        exit(EXIT_FAILURE);
    }
    for (iter = 0; iter < ELEMS; iter++) {
        ...
        data->key = iter;
        data->value = words[iter % size];
        data = hashmap_insert(map, data);
        if (data == NULL) {
    ...
    free(hashmap_delete(map, &temp));
    for (iter = 0; iter < 10; iter++) {
        temp.key = iter;
        temp.value = words[iter];
        data = hashmap_delete(map, &temp);
        if (data != NULL) {
            printf("%d %s deleted\n", data->key, data->value);
            free(data);
        }
